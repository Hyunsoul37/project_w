...
import { MdFilterAlt ,MdKeyboardArrowLeft,MdKeyboardArrowRight} from 'react-icons/md';
  ...
  const PageBtn=useCallback(()=>{
    ...
    for(let i = 0;i<10 ; i++){
      ...
      if(num < fulldata.length/20){
        pagebtns.push(<Link href={{pathname:`/product`, query:{page:num+1}}}>
        <span className={[styled.pagebtn,num===curpage?styled.activebtn:''].join(' ')} >{num+1}</span></Link>)
      }
    }
    return pagebtns;
  } ,[curpage])
  ...
  return (
    <div className={styled.ProductWrapper}>
      <div className="maxframe">
       ...
       {showFilter&&<ProductFilter isshow={showFilter} SetFilterData={SetFilterData} offFilter={offFilter} />}
      ...
      <div style={{ display: 'flex', justifyContent: 'center' ,flexDirection:'column',alignItems:'center'}}>
       ...
       {curpage > 9? <Link href={{pathname:`/product`, query:{page:Math.ceil((curpage-9)/10)*10}}}>
        <div className={styled.prevNextBtn}>
          <MdKeyboardArrowLeft />
          <span>이전</span>
        </div>
        </Link>:''}
       {PageBtn().map((c,idx)=><div key={`btn_${idx}`}className={styled.pagebtn}>{c}</div>)}
       {PageBtn().length < 10? '' :
       <Link href={{pathname:`/product`, query:{page:(Math.ceil((curpage+1)/10))*10+1}}}>
        <div className={styled.prevNextBtn}>        
          <span>다음</span>
